...
from database import Match
            ...
            COMPLETED_API_IDS = {
                ...
                "e02475c1-8f9a-4915-a9e8-d4dbc3441c96",
                "d788e9f9-99bf-4650-a035-92a7e21b3d08",
                "11ff7db9-9c71-464e-afcb-5b03e4fa4b0a",
                "ae676d7c-3082-489c-96c5-5620f393c900",
                "fd010e39-2255-4460-b0e0-962a26b67b70",
                "96d2aa6b-ea40-4da4-b4cf-eb996de24ef7",
                "736f3e02-212a-49bc-8b3b-08a106312702",
                "ea4d01bf-bf47-4f7d-a4f8-32eade678141",
            }
            ...
            with db.engine.connect() as conn:
                for match_num, api_id, team1, team2, date_str, venue in MATCHES:
                    ...
                    result = conn.execute(
                        text("SELECT id, status FROM match WHERE match_number = :n"),
                        {"n": match_num}
                    ).fetchone()

                    if result:
                        match_id, current_status = result[0], result[1]
                        new_status = current_status if current_status in ("completed", "live") else "upcoming"
                        conn.execute(text("""
                            UPDATE match SET
                                ...
                                status = :status
                            WHERE id = :id
                        """), {"api_id": api_id, "team1": team1, "team2": team2,
                               "match_date": match_date, "venue": venue,
                               "status": new_status, "id": match_id})
                        print(f"✅ Updated Match {match_num}: {team1} vs {team2} [{new_status}]")
                        updated += 1
                    else:
                        status = "completed" if api_id in COMPLETED_API_IDS else "upcoming"
                        conn.execute(text("""
                            INSERT INTO match
                                (match_number, cricapi_match_id, team1, team2,
                                 match_date, venue, status)
                            VALUES
                                (:num, :api_id, :team1, :team2,
                                 :match_date, :venue, :status)
                        """), {"num": match_num, "api_id": api_id, "team1": team1,
                               "team2": team2, "match_date": match_date,
                               "venue": venue, "status": status})
                        print(f"➕ Created Match {match_num}: {team1} vs {team2}")
                        created += 1
                conn.commit()

            total = conn.execute(text("SELECT COUNT(*) FROM match")).fetchone()[0] if False else Match.query.count()
            print(f"\n🏏 Done! Updated: {updated}, Created: {created}, Total in DB: {total}")
