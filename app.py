...
def scrape_confirm():
    ...
    flash(msg, "success")
    return redirect(url_for("admin"))
@app.route("/admin/import-excel/<int:match_id>", methods=["POST"])
@admin_required
def import_excel(match_id):
    match = Match.query.get(match_id)
    if not match:
    ...
    existing = {s.player_id: s for s in
                PlayerMatchStats.query.filter_by(match_id=match_id).all()}

    success_count = 0
    not_found     = []

    for row in rows:
        # Normalize keys — handle both exact and case-insensitive column names
        row = {str(k).strip(): v for k, v in row.items()}

        def get(keys, default=0):
            for k in keys:
                if k in row:
                    try: return float(row[k]) if '.' in str(row[k]) else int(row[k])
        ...
        if not player:
            for db_name, db_player in player_map.items():
                if player_name.lower() in db_name or \
                   db_name in player_name.lower():
                    player = db_player
                    break
        ...
        stats_dict = {
            "runs":          get(["Runs", "runs"]),
            "balls_faced":   get(["Balls", "balls_faced", "Balls Faced"]),
            "fours":         get(["4s", "fours"]),
            "sixes":         get(["6s", "sixes"]),
            "wickets":       get(["Wkts", "wickets"]),
            "overs_bowled":  get(["Overs", "overs_bowled"]),
            "runs_conceded": get(["Runs Con.", "runs_conceded", "RunsCon"]),
            "maidens":       get(["Maidens", "maidens"]),
            "wides":         get(["Wides", "wides"]),
            "no_balls":      get(["NB", "no_balls"]),
            "catches":       get(["Catches", "catches"]),
            "stumpings":     get(["Stump", "stumpings"]),
            "run_outs":      get(["R.Out", "run_outs"]),
            "is_motm":       bool(get(["MOTM", "is_motm"])),
            "is_winner":     bool(get(["Winner", "is_winner"])),
            "did_play":      True,
            "role":          player.role
        }

        save_player_stats(player, match_id, stats_dict, existing)
        success_count += 1

    db.session.commit()
    recalculate_user_points(match_id)
    match.status = "completed"
    db.session.commit()

    msg = f"Imported {success_count} players successfully!"
    if not_found:
...
def admin_debug_run():
    ...
    try:
        with redirect_stdout(output):
                ...
                # Try fetching scorecard directly with series_info IDs we have
                match_ids_to_check = [
                    ("19", "36d875e2-3333-4fab-ba4d-4f89fb4d7055", "LSG vs GT"),
                    ("30", "f72cb89b-3085-4556-a814-d28a44885b0e", "GT vs MI"),
                ]

                for match_num, match_id, name in match_ids_to_check:
                    r = requests.get(
                        "https://api.cricapi.com/v1/match_scorecard",
                        params={"apikey": API_KEY, "id": match_id}
                    )
                    data = r.json()
                    print(f"Match {match_num} ({name}): {data.get('status')} — {match_id}")
                    if data.get("status") == "success":
                        print(f"  ✅ ID works!")
                    else:
                        print(f"  ❌ ID doesn't work: {data.get('info', '')}")
                    print()
...
#--- temp start
@app.route("/ad     min/import-excel-debug/<int:match_id>", methods=["POST"])
@admin_required
def import_excel_debug(match_id):
    rows = request.get_json().get("rows", [])
    if not rows:
        return jsonify({"error": "no rows"})
